...
{
    public static MemoryTensorOwner<T> AllocateTensor<T>(this IMemoryAllocatorService allocator, TensorShape shape, bool clean = true) where T : unmanaged, INumber<T>
    {
        var memory = allocator.Allocate<T>(shape.Length, clean);
        return new MemoryTensorOwner<T>(memory, shape.Dimensions);
    }

    public static MemoryTensorOwner<float> AllocateTensor(this IMemoryAllocatorService allocator, TensorShape shape, bool clean = true)
    {
